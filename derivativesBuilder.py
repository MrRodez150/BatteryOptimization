...
def array_update(state, update_element):
    ...
    #return (J.at[].set(element),start_index,row),ind
...
def build_dup(J, partial, Ma, Mp):
    ranger_p = jnp.arange(1, Mp)
    ranger_m = jnp.arange(0, Mp - 1)
    # dudTp
    ...
    # TODO
    J=J.at[3, Ma + 1 + 3].set(partial[3][0])
...
def build_bc_p(J, bc, Ma, Mp):
    ...
    col_phis = p0 + jnp.array([1, 5 + 3, 5 + 6*(Mp-1) + 3, 5 + 6*Mp + 1])
    ...
    col_phie = p0 + jnp.array([2, 5 + 4, 5 + 6*(Mp-1) + 4, 5 + 6*Mp +2,
    ...
    col_T = jnp.array([Ma, Ma + 1 + 4, Ma + 1 + 3, Ma + 1 + 10, 5 + 6*(Mp-1)+5+Ma+1, 5+6*Mp+3+Ma+1, Ma + 1 + sep_o + 2, Ma + 1 + sep_o +2 + 3])
...
def build_bc_s(J, bc, Ma, Mp, Ms):
    ...
    col_u = jnp.array([sep0, sep0 + 3, 5 + 6*(Mp-1) + p0, 5+6*Mp + p0, n0, n0 + 4, sep0 + 3*Ms, sep0 + 3*(Ms+1) ])
    J=J.at[row_u, col_u].set(bc)
    #J = jax.ops.index_update(J, jax.ops.index[row_u, col_u], bc)

    row_phie = jnp.array([18, 14, 11, 8, 7, 1, 14, 11])
    col_phie = jnp.array([5+6*(Mp-1) + 4 + p0, 5 + 6*Mp +2 + p0, sep0+1, sep0 + 1 + 3, n0+2, n0+4 + 4, sep0 + 3*Ms + 1, sep0 + 3*Ms + 4])
    ...
    col_T = jnp.array([5 + 6*(Mp-1) + 5 + p0, 5 + 6*Mp + 3 + p0, sep0 + 2, sep0 + 5, sep0 + 3*Ms + 2, sep0 + 3*Ms + 5, n0 + 3, n0 + 4 + 5])
...
def build_bc_n(J, bc, Ma, Mp, Ms, Mn):
    ...
    col_phie = jnp.array([n0 + 2, n0 + 8, sep0 + 3*Ms + 1, sep0 + 3*Ms + 4,
    ...
    col_phis = jnp.array([n0 + 1, n0 + 7, n0 + 4 + 6*(Mn-1) + 3, n0 + 4 + 6*Mn + 1])
    J=J.at[row_phis, col_phis].set(bc['phis'])
    #J = jax.ops.index_update(J, jax.ops.index[row_phis, col_phis], bc['phis'])

    row_T = jnp.array([18,15,11, 5, 15, 11, 10, 9])
    col_T = jnp.array([sep0 + 3*Ms + 2, sep0 + 3*Ms + 5, n0 + 3, n0 + 9,
...
def build_dun(J, partial, Ma, Mp, Ms, Mn):
    p0 = Ma + 1
    n0 = p0 + 4 * (Mp + 2) + 2 * Mp + 3 * (Ms + 2) + 1
    ranger_p = jnp.arange(1, Mn)
    ranger_m = jnp.arange(0, Mn - 1)
    # dudTp
...
def build_bc_cc(J, bc, Ma, Mp, Ms, Mn, Mz):
    ...
    col_dTa = jnp.array([0, 1, Ma, 4 + p0, 3 + p0, 10 + p0])
    ...
    col_dTz = jnp.array([n0 + 4 + 6*(Mn-1)+5, n0 + 4 + 6*Mn + 3,n0 + 4 + 6*Mn + 4, n0 + 4 + 6*Mn + 5,
