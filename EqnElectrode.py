...
from settings import delta_t, dxO, dxA, dxZ, drP, drN, dxP, dxN
...
class ElectrodeEquation:   
    ...
    def __init__(self, constants, s_constants, a_constants, z_constants):
        ...
        self.sep_hx = self.sep.l/dxO
        
        self.acc = a_constants
        self.acc_hx = self.acc.l/dxA
        
        self.zcc=z_constants
        self.zcc_hx = self.zcc.l/dxZ
        ...
        self.rpts_end = jnp.arange(0,N)*self.Rp/N
        self.rpts_mid = self.Rp*(jnp.arange(0,N)-0.5)/N
        
        self.rpts_end = self.Rp*jnp.linspace(0,N, N+1)/N
        self.rpts_mid = self.Rp*(jnp.linspace(1,N, N)-0.5)/N
        R = self.Rp*(jnp.linspace(1,N,N)-(1/2))/N;
        r = self.Rp*(jnp.linspace(0,N, N+1))/N
    ...
    def Qohm(self,phisn, phisp, phien, phiep, un, up, uc, T):
        ...
        (2*kapeff*R*T/F)*(1-t_plus)*( (jnp.log(up) - jnp.log(un))/(2*hx) )*( (phiep - phien)/(2*hx) )
    ...
    def electrolyte_conc(self,un, uc, up, Tn, Tc, Tp, j,uold):
        ...
        ans = (uc-uold) - (self.delta_t/eps)*( ( Deff_r*(up - uc)/hx - Deff_l*(uc - un)/hx )/hx + a*(1-t_plus)*j ) 
    ...
    def electrolyte_poten(self,un, uc, up, phien, phiec, phiep, Tn, Tc, Tp,j):
        ...
        - gamma*(kapeff_r*Tmid_r*(jnp.log(up) - \
                                               jnp.log(uc))/hx  \
            - kapeff_l*Tmid_l*(jnp.log(uc) - \
                                               jnp.log(un))/hx )/hx
        return ans.reshape()
...
#        a = self.a;
    ...
    def ionic_flux(self,j,u,T,eta,cM, cMp,cmax):
        ...
        ans = j - 2*keff*jnp.sqrt(u*(cmax - cs)*cs)*term2
        return ans.reshape()
    ...
    def ionic_flux_phi(self,j,u,T,phis,phie,cM, cMp,cmax):
        cs = (cM+cMp)/2
        keff = self.k*jnp.exp( (-self.Ek/R)*((1/T) - (1/T_ref)))
        ans = j - 2*keff*jnp.sqrt(u*(cmax - cs)*cs)*jnp.sinh( (0.5*F/(R*T))*(self.over_poten_exp(phis,phie, T,  cM, cMp, cmax)) )
        return ans.reshape()
    ...
    def open_circuit_poten(self,cM, cMp,T,cmax):
        ...
        ans = Uref + (T - T_ref)*self.entropy_change(cM, cMp,T,cmax)
        return ans
    
    def open_circuit_poten_fast(self,cs,T,cmax):
        Uref = self.open_circ_poten_ref_fast(cs,T,cmax)
        ans = Uref + (T - T_ref)*self.entropy_change_fast(cs,T,cmax)
        return ans
